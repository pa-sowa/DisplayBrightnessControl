...
Item {
    ...
    Plasmoid.icon: {
        source: {
            if (plasmoid.configuration.slimIcon)
            ...
            Controls1.Slider {
                ...
                onPressedChanged: {
                    if (!pressed) {
                        ...
                        if (realValue >= 0) {
                            dimmed = false
                            if (realValue > plasmoid.configuration.limitBrightnessMax)
                                realValue = plasmoid.configuration.limitBrightnessMax
                            if (realValue < plasmoid.configuration.limitBrightnessMin)
                                realValue = plasmoid.configuration.limitBrightnessMin
                            ...
                            cmd.exec('xrandr --output "' + plasmoid.configuration.output
                                     + '" --brightness 1.0;')
                        } else {
                            var floatValue = -realValue
                            var ranValue = floatValue / brigthnessMax
                            ranValue = (1.000 - ranValue).toFixed(3)
                            if (ranValue < plasmoid.configuration.limitDimness)
                                ranValue = plasmoid.configuration.limitDimness
            ...
            onClicked: {
                if (mouse.button === Qt.LeftButton) {
                    if (plasmoid.expanded) {
                        ...
                        if (powerMan.data["PowerDevil"]
                                && powerMan.data["PowerDevil"]["Screen Brightness Available"]) {
                            var maxBr = (powerMan.data["PowerDevil"]["Maximum Screen Brightness"])
                            if (maxBr > plasmoid.configuration.limitBrightnessMax)
                                maxBr = plasmoid.configuration.limitBrightnessMax
                            brigthnessMax = maxBr
    ...
    Plasmoid.toolTipSubText: {
        "Check settings for safe limits."
    }
}
